...
public class Addgd implements Initializable {
    public TextField addgv, addlh, addmh;
    ...
    public void Addgd(MouseEvent event) throws Exception {
        String gv = addgv.getText();
        if (gv == null) gv = "";
        gv = MuonTra.toUpper(gv);
        String lh = addlh.getText();
        if (lh == null) lh = "";
        lh = MuonTra.toUpper(lh);
        String mh = addmh.getText();
        if (mh == null) mh = "";
        if ((gv.length() < 1) | (lh.length() < 1) | (mh.length() < 1)) {
        ...
        strsql = "INSERT INTO giang_day (MaGV, MaLop, MonHoc) VALUES ('" + gv + "', '" + lh + "', '" + mh + "')";
        comm.execute(strsql);
